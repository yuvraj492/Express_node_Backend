...
const updatUserCoverImage=asyncHandler(async(req,res)=>{
    ...
    const coverImage=await uploadOnCloudinary(coverImageLocalPath)
    if(!coverImage.url){
        throw new ApiError(400,"Error file uploading on coverImage")
    }
    const user=await User.findByIdAndUpdate(req.user?._id,{
        $set:{
            coverImage:coverImage.url
...
const getWatchHistory=asyncHandler(async(req,res)=>{
    ...
    return res.status(200).json(new ApiResponse(200,user[0].getWatchHistory,"watchHistory fetched successfully"))
})
